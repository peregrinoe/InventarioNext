...
function abrirNuevaAsignacion() {
    ...
    if (btnSubmit) {
        btnSubmit.innerHTML = '<i data-lucide="save" style="width:14px;height:14px;vertical-align:middle;margin-right:5px;"></i> Asignar Equipo';
    }
...
function renderAsignaciones() {
    ...
    const searchTerm      = (document.getElementById('searchAsignacion')?.value        || '').toLowerCase().trim();
    const filterEstado    =  document.getElementById('filterEstadoAsignacion')?.value   || '';
    const filterTipo      =  document.getElementById('filterTipoAsignacion')?.value     || '';

    const asignacionesFiltradas = database.asignaciones.filter(asig => {
        ...
        const hayEstado = !filterEstado || asig.estado === filterEstado;
        const hayTipo   = !filterTipo   || (filterTipo === 'Temporal' ? asig.esTemporal : !asig.esTemporal);

        return hayBusqueda && hayEstado && hayTipo;
    });
    ...
    tbody.innerHTML = asignacionesFiltradas.map(asig => {
        ...
        const tipoBadge = asig.esTemporal
            ? '<span class="badge badge-warning"><i data-lucide="clock" style="width:11px;height:11px;vertical-align:middle;margin-right:2px;"></i>Temporal</span>'
        ...
        return `
            <tr>
                ...
                <td><code style="font-size:12px;">${equipo?.numSerie || '-'}</code></td>
                <td>${tipoBadge}</td>
                ...
                <td class="action-buttons">
                    <div class="action-buttons-inner">
                        <button class="btn btn-sm btn-primary" onclick='editAsignacion("${asig._id}")' title="Editar asignación"><i data-lucide="pencil"></i> Editar</button>
                        ${asig.estado === 'Activa' ? 
                            `<button class="btn btn-sm btn-warning" onclick='devolverEquipo("${asig._id}")' title="Devolver equipo"><i data-lucide="rotate-ccw"></i> Devolver</button>` : 
                            ''
                        }
                        ${asig.estado === 'Activa' && colaborador
                            ? `<button class="btn btn-sm btn-info carta-responsiva" onclick='descargarCartaDevolucion("${colaborador._id}")' title="Carta de devolución"><i data-lucide="file-check"></i> Carta Devolución</button>`
                            : ''
                        }
                        ${asig.esTemporal && asig.estado === 'Activa' && colaborador
                            ? `<button class="btn btn-sm btn-info carta-responsiva" onclick='descargarCartaTemporal("${colaborador._id}","${asig._id}")' title="Carta responsiva temporal"><i data-lucide="file-text"></i> Carta Temporal</button>`
                            : ''
                        }
                        <button class="btn btn-sm btn-danger" onclick='deleteAsignacion("${asig._id}")' title="Eliminar asignación"><i data-lucide="trash-2"></i></button>
                    </div>
                </td>
            </tr>
        `;
...
function editAsignacion(id) {
    ...
    const btnSubmit = document.querySelector('#modalAsignacion .btn-success');
    if (btnSubmit) {
        btnSubmit.innerHTML = '<i data-lucide="save" style="width:14px;height:14px;vertical-align:middle;margin-right:5px;"></i> Actualizar Asignación';
    }
