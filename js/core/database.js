 ...
 * texto de usuarios en innerHTML / template literals.
 * @param {any} value
 * @returns {string}
 */
...
/**
 * Procesa los elementos data-lucide recién insertados en el DOM.
 * Debe llamarse después de cualquier innerHTML que incluya iconos.
 */
...
/**
 * Valida un conjunto de campos. Lanza Error con mensaje si falla.
 * @param {Object} fields  — { label: value }
 * @param {Object} rules   — { label: { required?, email?, phone?, maxLen? } }
 */
function validateFields(fields, rules) {
    ...
    for (const [label, rule] of Object.entries(rules)) {
        ...
        if (!val) continue;  // campo opcional vacío — no validar más

        if (rule.email && !emailRe.test(val)) {
            errors.push(`"${label}" no tiene un formato de correo válido.`);
        }
        if (rule.phone && !phoneRe.test(val)) {
            errors.push(`"${label}" solo puede contener dígitos, espacios, guiones y paréntesis.`);
        }
        if (rule.maxLen && val.length > rule.maxLen) {
            errors.push(`"${label}" no puede superar ${rule.maxLen} caracteres.`);
        }
    }
...
async function loadData() {
    ...
    try {
        const [
            ...
            accesoriosRes, asignacionesAccesoriosRes
        ] = await Promise.all([
            ...
            supabaseClient.from('accesorios').select('*').order('tipo').order('marca'),
            supabaseClient.from('asignaciones_accesorios').select('*')
        ]);
        ...
        database.accesorios            = (accesoriosRes.data || []).map(mapAccesorio);
        database.asignacionesAccesorios = (asignacionesAccesoriosRes.data || []).map(mapAsignacionAccesorio);
...
function mapEquipo(row) {
    ...
    return {
        ...
        owner: row.owner || '',
        esquema: row.esquema || '',
        cifradoDisco: row.cifrado_disco || '',
        antivirusEdr: row.antivirus_edr || '',
...
function mapAsignacionAccesorio(row) {
    return {
        _id: row.id,
        colaboradorId: row.colaborador_id,
...
function saveData() {
    console.log('ℹ️ saveData() — los datos se guardan directamente en Supabase');
}

// --- COLABORADORES ---
async function upsertColaborador(colaborador) {
    const row = {
        id: colaborador._id, nombre: colaborador.nombre,
        email: colaborador.email, telefono: colaborador.telefono || null,
        departamento: colaborador.departamento, puesto: colaborador.puesto,
        ...
        es_activo: colaborador.esActivo !== false,
...
async function upsertEquipo(equipo) {
    const row = {
        id: equipo._id, marca: equipo.marca, modelo: equipo.modelo,
        tipo: equipo.tipo || null, num_serie: equipo.numSerie || null,
        nombre_equipo: equipo.nombreEquipo || null, id_interno: equipo.idInterno || null,
        procesador: equipo.procesador || null, ram: equipo.ram || null,
        almacenamiento: equipo.almacenamiento || null, so: equipo.so || null,
        estado: equipo.estado || 'Disponible', condicion: equipo.condicion || null,
        ubicacion: equipo.ubicacion || null, categoria: equipo.categoria || null,
        propiedad: equipo.propiedad || null, precio: equipo.precio || null,
        fecha_compra: equipo.fechaCompra || null, proveedor: equipo.proveedor || null,
        factura: equipo.factura || null, garantia: equipo.garantiaMeses || null,
        ...
        owner: equipo.owner || null, esquema: equipo.esquema || null,
...
async function upsertCelular(celular) {
    const row = {
        id: celular._id, marca: celular.marca, modelo: celular.modelo,
        imei: celular.imei || null, numero: celular.numero || null,
        compania: celular.compania || null, num_serie: celular.numSerie || null,
        propiedad: celular.propiedad || null, almacenamiento: celular.almacenamiento || null,
        color: celular.color || null, so: celular.so || null, plan: celular.plan || null,
        costo_plan: celular.costoPlan || null, renovacion_plan: celular.renovacionPlan || null,
        estado: celular.estado, condicion: celular.condicion || null,
        precio: celular.precio || null, fecha_compra: celular.fechaCompra || null,
        proveedor: celular.proveedor || null, factura: celular.factura || null,
        garantia_meses: celular.garantiaMeses || null,
        fotos: celular.fotos ? JSON.stringify(celular.fotos) : null,
        observaciones: celular.observaciones || null, created_at: celular.createdAt
    };
...
async function upsertAsignacion(asig) {
    const row = {
        id: asig._id, colaborador_id: asig.colaboradorId, equipo_id: asig.equipoId,
        fecha_asignacion: asig.fechaAsignacion, fecha_devolucion: asig.fechaDevolucion || null,
        estado: asig.estado, notas: asig.notas || null,
        es_temporal: asig.esTemporal || false, fecha_fin_temporal: asig.fechaFinTemporal || null
    };
...
async function upsertAsignacionCelular(asig) {
    const row = {
        id: asig._id, colaborador_id: asig.colaboradorId, celular_id: asig.celularId,
        fecha_asignacion: asig.fechaAsignacion, fecha_devolucion: asig.fechaDevolucion || null,
        estado: asig.estado, notas: asig.notas || null,
        es_temporal: asig.esTemporal || false, fecha_fin_temporal: asig.fechaFinTemporal || null
    };
...
async function upsertLicencia(lic) {
    const row = {
        id: lic._id, software: lic.software, tipo: lic.tipo,
        clave: lic.clave || null, fecha_compra: lic.fechaCompra || null,
        fecha_vencimiento: lic.fechaVencimiento || null,
        estado: lic.estado, notas: lic.notas || null, created_at: lic.createdAt
    };
...
async function upsertLicenciaAsignacion(la) {
    const row = {
        id: la._id, licencia_id: la.licenciaId,
        colaborador_id: la.colaboradorId, fecha_asignacion: la.fechaAsignacion
    };
...
async function deleteLicenciaAsignacionesPorLicencia(licenciaId) {
    const { error } = await supabaseClient.from('licencias_asignaciones').delete().eq('licencia_id', licenciaId);
    if (error) throw error;
}

// --- ACCESORIOS ---
async function upsertAccesorio(acc) {
    const row = {
        id: acc._id, tipo: acc.tipo, marca: acc.marca, modelo: acc.modelo || null,
        num_serie: acc.numSerie || null, id_interno: acc.idInterno || null,
        propiedad: acc.propiedad || 'Empresa', condicion: acc.condicion || null,
        estado: acc.estado || 'Disponible', observaciones: acc.observaciones || null,
        created_at: acc.createdAt || new Date().toISOString()
    };
    const { error } = await supabaseClient.from('accesorios').upsert(row, { onConflict: 'id' });
    if (error) throw error;
}

async function deleteAccesorioDB(id) {
    const { error } = await supabaseClient.from('accesorios').delete().eq('id', id);
    if (error) throw error;
}

async function upsertAsignacionAccesorio(asig) {
    const row = {
        id: asig._id, colaborador_id: asig.colaboradorId, accesorio_id: asig.accesorioId,
        fecha_asignacion: asig.fechaAsignacion, fecha_devolucion: asig.fechaDevolucion || null,
        estado: asig.estado, notas: asig.notas || null
    };
    const { error } = await supabaseClient.from('asignaciones_accesorios').upsert(row, { onConflict: 'id' });
    if (error) throw error;
}

async function deleteAsignacionAccesorioDB(id) {
    const { error } = await supabaseClient.from('asignaciones_accesorios').delete().eq('id', id);
    if (error) throw error;
}

// --- BAJAS ---
async function upsertSolicitudBaja(solicitud) {
    const row = {
        id: solicitud._id, colaborador_id: solicitud.colaboradorId,
        motivo: solicitud.motivo, fecha: solicitud.fecha || null,
        estado: solicitud.estado || 'pendiente',
        solicitado_por: solicitud.solicitadoPor || null,
        rol_solicitante: solicitud.rolSolicitante || null,
        fecha_solicitud: solicitud.fechaSolicitud || new Date().toISOString(),
        fecha_resolucion: solicitud.fechaResolucion || null,
        resuelto_por: solicitud.resueltoPor || null,
        notas_resolucion: solicitud.notasResolucion || null
    };
    const { error } = await supabaseClient.from('solicitudes_baja').upsert(row, { onConflict: 'id' });
    if (error) throw error;
}

async function upsertHistorialBaja(baja) {
    const row = {
        id: baja._id, colaborador_id: baja.colaboradorId,
        fecha_baja: baja.fechaBaja || null, motivo: baja.motivo || null,
        receptor_id: baja.receptorId || null, procesado_por: baja.procesadoPor || null,
        activos_reasignados: baja.activosReasignados || 0,
        created_at: baja.createdAt || new Date().toISOString()
    };
...
async function importarDatos(event) {
    ...
    reader.onload = async function(e) {
        try {
            ...
            showNotification('⏳ Importando datos...', 'success');
...
function renderAll() {
    ...
    try { if (typeof renderAccesorios === 'function') renderAccesorios(); } catch(e) {}
...
// Modales de captura que NO cierran al hacer clic en el fondo (solo con X o Cancelar)
const MODALES_SIN_CIERRE_FONDO = ['modalColaborador', 'modalEquipo', 'modalCelular', 'modalLicencia', 'modalAccesorio'];
